...
const TimelineBlock: React.FC<TimelineBlockProps> = ({
  ...
  return (
    <div
      className={`${width} flex items-center ${backgroundColor} text-white rounded-full py-1 px-2 space-x-1`}
    >
      <span className="w-1.5 h-1.5 bg-white rounded-full"></span>
      <span className="text-xs truncate">{title}</span>
      <div
        className={`${percentageColor} rounded-full py-0.5 px-1 text-xs ml-auto`}
      >
        {percentage}%
      </div>
        ...
        <div className="inline-block min-w-full">
          <div className="flex justify-between mb-2">
            {dates.map((date, index) => (
              <div
                key={index}
                className="text-xs text-gray-600 w-12 flex-shrink-0"
              >
                {date}
              </div>
            ))}
          ...
          <div className="relative h-[500px]">
            <div className="absolute top-0 left-0 right-0 bottom-0 grid grid-cols-21 gap-0">
              {Array.from({ length: 21 }).map((_, index) => (
                <div
                  key={index}
                  className="border-l border-dashed border-gray-300 h-full"
                ></div>
              ))}
            </div>
