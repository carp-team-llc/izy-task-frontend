...
export default function Component() {
  ...
  const { data: todayTask, isLoading } = useTodayTask({ projectId, today });
  ...
  return (
    <div className="col-span-2 bg-[#130b3b] rounded-lg p-4">
      ...
      <div className="space-y-3">
        {Array.isArray(todayTask) && todayTask.length > 0 ? (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                  <img src={task.employee?.profile?.avatar} className="rounded-full w-6 h-6 object-cover" alt="avatar" />
                </div>
              </div>
            </div>
          ))
        ) : (
          <div className="text-center text-gray-400 py-4">No tasks for today</div>
        )}
      </div>
    </div>
  );
